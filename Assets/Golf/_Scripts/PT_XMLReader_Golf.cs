...
[System.Serializable]
public class PT_XMLReader_Golf {
	...
	*/
	...
	public void Parse(string eS) {
		xmlText = eS;
		xml = new PT_XMLHashtable_Golf();
		Parse(eS, xml);
	}
	
	// This function will parse a possible series of tags
	void Parse(string eS, PT_XMLHashtable_Golf eH) {
		eS = eS.Trim();
		while(eS.Length > 0) {
			eS = ParseTag(eS, eH);
			eS = eS.Trim();
	...
	string ParseTag(string eS, PT_XMLHashtable_Golf eH) {
		// search for "<"
		int ndx = eS.IndexOf("<");
		...
		if (!eH.ContainsKey(tag)) {
			eH[tag] = new PT_XMLHashList_Golf();
		}
		// Create a hashtable to contain this tag's information
		PT_XMLHashList_Golf arrL = eH[tag] as PT_XMLHashList_Golf;
		//int thisHashIndex = arrL.Count;
		PT_XMLHashtable_Golf thisHash = new PT_XMLHashtable_Golf();
		arrL.Add(thisHash);
...
public class PT_XMLHashList_Golf {
	public ArrayList list = new ArrayList();
	
	public PT_XMLHashtable_Golf this[int s] {
		get {
			return(list[s] as PT_XMLHashtable_Golf);
		}
		set {
			list[s] = value;
		}
	...
	public void Add(PT_XMLHashtable_Golf eH) {
		list.Add(eH);
	}
...
public class PT_XMLHashtable_Golf {
	
	public List<string>				keys = new List<string>();
	public List<PT_XMLHashList_Golf>		nodesList = new List<PT_XMLHashList_Golf>();
	public List<string>				attKeys = new List<string>();
	public List<string>				attributesList = new List<string>();
	
	public PT_XMLHashList_Golf Get(string key) {
		int ndx = Index(key);
		if (ndx == -1) return(null);
		return( nodesList[ndx] );
	}
	
	public void Set(string key, PT_XMLHashList_Golf val) {
		int ndx = Index(key);
		if (ndx != -1) {
			nodesList[ndx] = val;
		} else {
	...
	public PT_XMLHashList_Golf this[string s] {
		get {
			return( Get(s) );
		}
		set {
